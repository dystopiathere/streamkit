...
/**
 * Согласия при регистрации. Каждое поле обязано быть true — 152-ФЗ требует
 * активного действия пользователя, предустановленная галочка согласием не считается.
 * Версии принятых документов проставляет бэкенд из актуального реестра.
 */
export const registerConsentsSchema = z.object({
  terms: z.literal(true),
  privacy: z.literal(true),
  personalData: z.literal(true),
});
export type RegisterConsents = z.infer<typeof registerConsentsSchema>;

export const registerSchema = z.object({
  email: emailSchema,
  password: passwordSchema,
  displayName: z.string().trim().min(2).max(40),
  consents: registerConsentsSchema,
});
