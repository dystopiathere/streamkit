...
/**
 * Видео ли по ссылке. Проверяется путь, а не строка целиком: у ссылки бывают
 * параметры (`clip.webm?v=2`), и расширение в конце строки их бы не пережило.
 *
 * Только WebM: у него есть прозрачность (альфа-канал VP8/VP9), ради которой
 * анимированные оповещения и делают видео вместо GIF, и его играет любой
 * Chromium, включая браузер-сорс OBS. Здесь, а не в рендерере: от ответа
 * зависит и то, откуда оверлей берёт звук оповещения.
 */
export function isVideoUrl(url: string): boolean {
  // Без `URL`: контракты собираются без типов DOM и Node. Отрезаются запрос и
  // якорь, а расширение ищется только в пути — после хоста.
  const [address = ''] = url.split(/[?#]/, 1);
  return /^[a-z][a-z0-9+.-]*:\/\/[^/]+\/.*\.webm$/i.test(address);
}
