...
export const LEGAL_DOCUMENTS: Record<ConsentDocument, LegalDocument> = {
  TERMS: {
    document: 'TERMS',
    // Пятая редакция: окно эфира — чат подключённого канала Twitch и показ чата
    // самому стримеру в личном кабинете (разделы 13.2 и 13.3). Четвёртая — что
  ...
  PRIVACY: {
    document: 'PRIVACY',
    // Восьмая редакция: время начала эфира от площадок и окно эфира (разделы 2,
    // 4.2, 5.1, 5.2). Седьмая — данные из Google (раздел 5) — что берём у YouTube,
