 ...
 * механизма команд воркеру в проекте нет вовсе — `ConnectorManager` поднимает
 * донат-коннекторы один раз на старте и реагирует на изменение источника только
 * перезапуском. Для чата так нельзя: стример создал виджет и ждёт чат сейчас, а
 * не после следующего деплоя. Один лёгкий запрос раз в десять секунд дешевле,
 * чем заводить канал команд ради единственного потребителя.
 */
@Injectable()
