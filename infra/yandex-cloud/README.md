...
terraform apply -var registry_cleanup=null
```

Первый `apply` — с `registry_cleanup=null`: уборка образов вешается на
репозитории реестра, а они появляются только с первым выпуском (раздел 6).
После первого выпуска — обычный `terraform apply`, он включит уборку.
...
Счёт почти целиком — вычислительные ресурсы, работающие круглосуточно:
управляемые PostgreSQL и Valkey, затем две ВМ. Трафик, DNS, Lockbox и реестр —
единицы процентов. Точная разбивка — «Биллинг → Детализация» по сервисам;
сверять с ней, а не с калькулятором.

Размеры по умолчанию рассчитаны на этап до первых платящих стримеров:
...
Всё меняется в `terraform.tfvars` и `terraform apply` без переноса данных.
Смена класса базы из одного хоста — несколько минут простоя, ВМ — перезапуск.

Дальше рост упирается в архитектуру, а не в размеры, и это отдельные решения:

- **Второй экземпляр API.** Код к этому готов: блокировки, лимиты, присутствие
  оверлеев и шина событий живут в Valkey, а не в памяти процесса. Нужны вторая
  ВМ и балансировщик перед ними (Network Load Balancer), а сертификаты Caddy —
  в общем хранилище или на Application Load Balancer с Certificate Manager.
- **Отказоустойчивость баз** — второй хост кластера в другой зоне; стоит как
  ещё один кластер.

Минуты GitHub Actions: пулреквест, пока он черновик, CI не запускает; образы в
пулреквесте собираются только при правке Dockerfile'ов, зависимостей или схемы
Prisma; выпуск — одно задание. Подробности — в `infra/CLAUDE.md`.

## Защита от DDoS

В коде — то, что держит одиночный адрес и медленных клиентов; от
объёмной атаки, забивающей канал ВМ, код не спасает: пакеты доходят до
сетевой карты раньше, чем до Caddy. Это решает только защита на стороне облака.

Что есть в репозитории:

| Слой | Что ограничено | Где |
|---|---|---|
| Caddy | заголовки за 10 с (slowloris), простой keep-alive 2 мин, заголовки до 32 КБ, тело до 1 МБ (413) | `app/Caddyfile` |
| HTTP API | запросы в минуту на IP, счётчики в Valkey; вход и регистрация — 10 | `ThrottlerModule` в `app.module.ts` |
| Сокеты | новые подключения в минуту на IP (`SOCKET_HANDSHAKE_LIMIT`), открытые на IP (`SOCKET_CONNECTIONS_PER_IP`), сообщение до 16 КБ | `socket-admission.ts` |
| Вебхуки | Kick и ЮKassa сняты с лимитеров: их шлют пачками с немногих адресов. Поддельный Kick отсекает подпись, поддельная ЮKassa — только повод переспросить о платеже, который у нас в ожидании | контроллеры вебхуков |

Защита на стороне облака, по возрастанию цены (цены — прайс Yandex Cloud на
сентябрь 2026, с НДС; сверять с ним же перед решением):

1. **Базовая защита адреса (L3–L4).** Включается только при резервировании
   адреса: у нынешнего `streamkit-app` её не включить, нужен новый адрес с
   `ddos_protection_provider = "qrator"` и перенос A-записей — несколько минут
   недоступности, пока расходится DNS. Оплата — за очищенный входящий трафик,
   около 1,1 ₽/ГБ, атакующий трафик не оплачивается. Порог срабатывания
   настраивает поддержка под реальный трафик. То же стоит сделать для адреса
   LiveKit: UDP медиа — самая простая цель.
2. **Smart Web Security с защитой домена (L7).** Прокси облака перед Caddy:
   фильтр ботов, лимиты запросов по правилам, WAF. Минимум — 6 ресурсных
   единиц по 2,44 ₽/ч, это около 10 700 ₽ в месяц, плюс 1,1 ₽/ГБ трафика и
   оплата за каждый миллион проверенных запросов. Выпуск сертификатов
   переезжает к прокси, а адрес посетителя приходит заголовком — `trust proxy`
   и `clientIp` рассчитаны на один прокси и потребуют правки.
3. **Advanced DDoS Protection** — от 28 060 ₽ в месяц; оправдана, когда
   атаки уже идут и мешают платящим стримерам.
