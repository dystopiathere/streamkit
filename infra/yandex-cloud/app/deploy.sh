...
# Образы прошлых выпусков — ДО скачивания новых.
#
# `docker image prune -f` в конце скрипта убирает только образы без тега, а у
# прошлого выпуска тег есть — свой sha. Значит, каждая выкатка оставляла на
# диске полный набор образов (около полутора гигабайт), и через полтора десятка
# выпусков диск ВМ кончился: выкатка упала на `pull` с «no space left on
# device», не дойдя ни до миграций, ни до запуска.
#
# Чистим до скачивания: место нужно освободить раньше, чем оно понадобится.
...
# остаться на диске. Отказы поэтому и глушатся.
echo "==> Уборка прошлых выпусков"
df -h --output=avail /var/lib/docker 2>/dev/null | tail -1 | xargs echo "Свободно до уборки:"
docker images --format '{{.Repository}}:{{.Tag}}' |
  grep -E "^${REGISTRY}/[^:]+:[0-9a-f]{40}$" |
  grep -v ":${TAG}$" |
  xargs -r -n1 docker image rm >/dev/null 2>&1 || true
docker image prune -f >/dev/null
df -h --output=avail /var/lib/docker 2>/dev/null | tail -1 | xargs echo "Свободно после уборки:"
